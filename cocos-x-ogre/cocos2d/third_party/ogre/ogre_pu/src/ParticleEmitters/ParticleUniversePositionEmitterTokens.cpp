...
namespace ParticleUniverse
{
	...
	void PositionEmitterWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		// List of positions
		cache_std::vector<Vector3> positionList = emitter->getPositions();
		if(!positionList.empty())
		{
			cache_std::vector<Vector3>::const_iterator it;
			cache_std::vector<Vector3>::const_iterator itEnd = positionList.end();
