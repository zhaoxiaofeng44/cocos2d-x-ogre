...
namespace ParticleUniverse
{
	...
	void ParticleAffectorWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		const cache_std::list<String> excludedEmitters = affector->getEmittersToExclude();
		String emitterList = StringUtil::BLANK;
		if (!excludedEmitters.empty())
		{
			cache_std::list<String>::const_iterator it;
			cache_std::list<String>::const_iterator itEnd = excludedEmitters.end();
