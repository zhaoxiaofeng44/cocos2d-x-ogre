...
*/
	...
	//-----------------------------------------------------------------------
	void ParticleSystem::_updateTransformWithMatrixChange(void)
	{
		if(mParentNode)
		{
			mParticleSystemScale = mParentNode->_getDerivedScale();
			mParticleSystemScaleVelocity = mParticleSystemScale.x;
		}
	
		latestPosition = getDerivedPosition();
		mRotationCentre = latestPosition;

		mRotationOffset = getDerivedOrientation();
		mLatestOrientation = mRotationOffset; // Added for 1.3 (orientation not pr)
		mAtLeastOneParticleEmitted = false;
		_notifyMoved();
		_notifyRescaled();
		_notifyVelocityRescaled();
