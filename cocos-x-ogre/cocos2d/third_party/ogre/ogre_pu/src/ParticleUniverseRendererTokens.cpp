...
namespace ParticleUniverse
{
	...
	void ParticleRendererWriter::write(ParticleScriptSerializer* serializer, const IElement* element)
	{
		...
		const cache_std::vector<Ogre::FloatRect*> uvList = renderer->getTextureCoords();
		if (!uvList.empty())
		{
			serializer->writeLine(token[TOKEN_RENDERER_TEXCOORDS_SET], 12);
			serializer->writeLine("{", 12);
			cache_std::vector<Ogre::FloatRect*>::const_iterator it;
			cache_std::vector<Ogre::FloatRect*>::const_iterator itEnd = uvList.end();
