	...
	*/
	class _ParticleUniverseExport ParticleObserver : public IAlias, public IElement
	{
		public:
			...
			typedef cache_std::vector<ParticleEventHandler*>::iterator ParticleEventHandlerIterator;
			typedef cache_std::vector<ParticleEventHandler*>::const_iterator ParticleEventHandlerConstIterator;
			typedef cache_std::vector<ParticleEventHandler*> ParticleEventHandlerList;
		...
		protected:
			...
			*/
			...
			/** Determines whether mParticleTypeToObserve is set. If true? only that particles of the specified type
				are observed. If  alse?(= default), all particles are observed.
			*/
