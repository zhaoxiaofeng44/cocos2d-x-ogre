    ...
    */
	class _ParticleUniverseExport ParticleTechnique : public Particle, public IAlias, public IElement
	{
		public:
			...
			typedef cache_std::vector<ParticleEmitter*> ParticleEmitterList;
			typedef ParticleEmitterList::const_iterator ParticleEmitterIterator;

			typedef cache_std::vector<ParticleAffector*> ParticleAffectorList;
			typedef ParticleAffectorList::const_iterator ParticleAffectorIterator;

			typedef cache_std::vector<ParticleObserver*> ParticleObserverList;
			typedef ParticleObserverList::const_iterator ParticleObserverIterator;

			typedef cache_std::vector<Extern*> ExternList;
			typedef ExternList::const_iterator ExternIterator;

			typedef cache_std::vector<TechniqueListener*> TechniqueListenerList;
		...
		protected:
	        ...
	        */
			...
			@remarks
				...
				distance between the technique and the latest camera update and you will never know which
				camera it was.
				The reason to have this attribute is to be able to apply LOD to a Particle System and Particle
				System LOD is only effective with 1 camera (otherwise, with more camera there is a risk that 
